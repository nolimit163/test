...
class ShopqiAppGenerator < Rails::Generators::Base
  ...
  def add_config_variables
    api_key_str = api_key.nil? ? "ENV['SHOPQI_API_KEY']" : api_key.inspect
    api_secret_str = secret.nil? ? "ENV['SHOPQI_API_SECRET']" : secret.inspect
    
    inject_into_file 'config/application.rb', <<-DATA, :after => "class Application < Rails::Application\n"
    
    # Shopify API connection credentials:
    config.shopqi.api_key = #{api_key_str}
    config.shopqi.secret = #{api_secret_str}
